...
#backward.model3 = MASS::stepAIC(full.model, 
#scope = list(upper = ~ temp + RH + wind + X + Y + DMC + month + FFMC + day + ISI + rain + DC, lower = ~1), direction="backward",trace = TRUE)

#summary(backward.model3)
```
...
Após várias tentativas, o melhor modelo encontrado foi o backward, obtendo um R² de 0,0662, o que indica que o modelo encontrado consegue explicar 6,2% da variabilidade da variável resposta. As variáveis finais selecionadas pelo modelo foram: X, temp, wind, DMC, month e DC. 

A respeito das variáveis selecionadas, a que se mostrou mais relevante foi a "month", onde percebemos uma influência forte dessa variável quando ela possui o valor "dec", o que seriam o mês de dezembro.

Esse modelo obteve um erro residual padrão muito grande, se comparado ao intercepto encontrado, o que indica uma porcentagem de erro bastante alta.