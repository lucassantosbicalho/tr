---
title: "Análise do dataset Forest Fires com Regressão Linear Múltipla"
...
8. Rain: A quantidade de chuva em mm/m² acumulada nos 30 minutros antes da medição (entre 0 e 6.4).
...
O material [Applied Multivariate Analysis, Darren Homrighausen, PhD](https://darrenho.github.io/AMA/regression4example.pdf) utiliza um mecanismo para visualizar a média das áreas queimadas, conforme a localização X e Y no conjunto de dados, que pode ser vista a seguir:
...
**47,77%** dos registros no conjunto de dados são de áreas que não foram queimadas. 
...
forward.model = MASS::stepAIC(minimal.model,
                               scope = list(upper = ~  X + temp + RH + wind + DMC + month + Y + FFMC + day + ISI + rain + DC,  lower = ~1),
...
full.model = lm(lnArea ~  X + temp + RH + wind + DMC + month + Y + FFMC + day + ISI + rain + DC, data = tempdat)
...
stepwise.model = MASS::stepAIC(initial.model,
                                scope = list(upper = ~   X + temp + RH + wind + DMC + month + Y + FFMC + day + ISI + rain + DC,
                                             lower = ~1),
...
Após várias tentativas, o melhor modelo encontrado pelo método backward, obtendo um R² de 0,0662, o que indica que o modelo encontrado consegue explicar 6,2% da variabilidade da variável resposta. As variáveis finais selecionadas pelo modelo foram: X, temp, wind, DMC, month e DC. 

As variáveis significativas à pelo menos 10% de significância para o modelo foram X, temp, DMC e monthdec (mês de dez). As demais não foram significativas ao modelo.
A interpretação com relação à variável resposta ln(area + 1) pode não ter significado paupável devido à transformação logarítmica aplicada à variável, contudo, isso não impossibilita a percepção das variáveis que mais impactam para o comportamento estudado, que são as áreas queimadas.

## Interpretações das variáveis significativas para o modelo:

- O intercepto é de 0,1851, ou seja, o valor esperado para ln(area + 1) caso os coeficientes do modelo sejam zero.

- Mantendo as demais variáveis constantes, ln(area + 1) aumenta, em média, 0,0471 unidades para cada incremento da variável X.

- Mantendo as demais variáveis constantes, ln(area + 1) aumenta, em média, 0,0347 unidades para 1º na temperatura.

- Mantendo as demais variáveis constantes, ln(area + 1) aumenta, em média, 0041 unidades para cada uma unidade incrementada no índice DMC.

- Mantendo as demais variáveis constantes, ln(area + 1) aumenta, em média, 2.20 unidades se o mês for dezembro.
